      ...
      <form onSubmit={handleSubmit} className="mt-5 grid gap-4">
        <div>
          ...
          <input
            ...
            className="mt-2 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-2"
          />
        </div>
        ...
        <button
          disabled={saving}
          className="rounded-xl bg-indigo-600 px-5 py-2 font-semibold hover:bg-indigo-500 disabled:opacity-60"
        >
...
export default function Goals() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-6">
        {/* Actions row (keeps your Refresh + Create Goal buttons) */}
        <div className="flex flex-wrap gap-2 justify-end">
          <button
            onClick={fetchGoals}
            className="rounded-xl border border-slate-700 bg-slate-950 px-4 py-2 text-sm hover:bg-slate-800 transition"
          >
            Refresh
          </button>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
            ...
            <p className="mt-2 text-xl font-semibold">₹{totalTarget.toFixed(2)}</p>
          </div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
            <p className="text-sm text-slate-400">Total Saved</p>
            <p className="mt-2 text-xl font-semibold">₹{totalSaved.toFixed(2)}</p>
          </div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
        ...
        <div className="mt-6 rounded-2xl border border-slate-800 bg-slate-900 overflow-hidden">
          <div className="p-5 border-b border-slate-800 flex items-center justify-between">
            <h2 className="text-lg font-semibold">Goals History</h2>
          ...
          {loading ? (
            ...
            <div className="p-5 grid gap-4">
              {items
                ...
                .sort(
                  (a, b) =>
                    pct(b.currentAmount, b.targetAmount) -
                    pct(a.currentAmount, a.targetAmount)
                )
                .map((g) => {
                  const id = getId(g);
